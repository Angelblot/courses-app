...
export default function Habitudes({session=false}:{session?:boolean}){
 ...
 const items=p.produits.filter(p=>p.favorite&&rayonDepuisLibelle(p.category)===cat?.cle&&!(session&&w.manques?.[`produit:${p.id}`]&&manqueActif(w,`produit:${p.id}`)));
 ...
 return <SafeAreaView edges={session?[]:['top']} style={ui.screen}><View style={{padding:20,paddingBottom:8}}><Head title="Mes habitudes" back={!session}/><Text style={ui.subtitle}>Un rayon à la fois. Ajuste la quantité, puis fais glisser la carte.</Text></View>
 ...
 </ScrollView><View style={ui.footer}><Pressable accessibilityRole="button" onPress={()=>router.push(session?'/wizard/exceptions':'/ajout')} style={{minHeight:40,justifyContent:'center'}}><Text style={[ui.link,{textAlign:'center'}]}>Chercher un produit exceptionnel</Text></Pressable><Action onPress={()=>router.push(session?'/wizard/exceptions':'/liste')}>{session?'Continuer avec les achats exceptionnels':'Vérifier ma liste'}</Action></View></SafeAreaView>
}
